...
class TestDataBase(unittest.TestCase):
    ...
    def action(self, db):
        ...
        self.assertEqual(db.get_table("foo").find({"a": 1}), [record])
        record.update(a=2)
        ...
        record.update(b="2")
        self.assertEqual(db.get_table("foo").find({"a": 2}), [record for _ in range(10)])
        self.assertEqual(db.get_table("foo").delete({"a": 2}), 10)
        self.assertEqual(db.get_table("foo").find({"a": 2}), [])
